...
// - Active HIGH oder LOW kann ï¿½ber das FHEM-Webfrontend konfiguriert werden
// - Pullup kann ï¿½ber das FHEM-Webfrontend aktiviert werden
// - Erkennung von Doppelklicks
// - Zusï¿½tzliches Event beim Loslassen einer lang gedrï¿½ckten Taste
//
//*******************************************************************
...
#define IDENTIFY_LED 12     // Identify LED --> zusätzliche LED für Anzeige in Verteilung
...
void handleKeys() {
	...
	for (int i =  0; i < HMW_CONFIG_NUM_KEYS; i++) {
		...
		button[i].Update();
           ...
           // INPUT_LOCKED? -------------
      if(config.keys[i].input_locked) continue;   // Eingang locked wenn Bit = 1
           //----------------------------------
		...
		if (button[i].clicks != 0) {
			...
			if (button[i].clicks == 1) { // Einfachklick
				keyPressNum[i]++;
				hmwmodule->broadcastKeyEvent(i,keyPressNum[i], 2); // Taste gedrï¿½ckt
    		}
    		if (button[i].clicks >= 2) {	// Mehrfachklick
    				keyPressNum[i]++;
    				hmwmodule->broadcastKeyEvent(i,keyPressNum[i], 1); // Taste gedrï¿½ckt
	   		}
		...
		else if (keyState[i] < 0) {		// Taste ist oder war lang gedrï¿½ckt
	    	if (button[i].depressed == true) {	// ist noch immer gedrï¿½ckt --> alle 300ms senden
//				if(lastSentLong[i]) {
